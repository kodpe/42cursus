...
/*   Updated: 2022/05/25 15:56:45 by sloquet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * gnl buffer_size 1 free static each step
 */
static int	create_heredoc(int ac, char *stop)
{
	int		fd;
	char	*line;
	int		stoplen;
	...
	if (ac < 6)
		return (-1);
	fd = open(TMP_FILE_HEREDOC, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (0 > fd)
		return (-1);
	...
	while (1)
	{
		...
		if (!line)
			break ;
		if (!ft_strncmp(line, stop, stoplen) && line[stoplen] == '\n')
			break ;
...
t_pipex	*create_pipex(int ac, char **av, char **envp)
{
	...
	p->cmds = av + 2;
	p->heredoc = 0;
	if (0 == ft_strncmp(av[1], HEREDOC, ft_strlen(HEREDOC)))
	{
		if (0 > create_heredoc(ac, av[2]))
		{
			free(p);
			error_exit(E_HEREDOC, "Error: heredoc failed\n");
		}
		p->cmds = av + 3;
		p->heredoc = 1;
	}
