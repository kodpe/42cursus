...
int	main(void)
{
...
#endif
	{
		...
		rrf = someRandomIntern.makeForm("robotomy request", "Bendou");
		if (rrf)
		{
			boss.signForm(*rrf);
			boss.executeForm(*rrf);
		...
		rrf = someRandomIntern.makeForm("presidential pardon", "Benda");
		if (rrf)
		{
			boss.signForm(*rrf);
			boss.executeForm(*rrf);
		...
		rrf = someRandomIntern.makeForm("54986yhgj", "Benda");
		if (rrf)
		{
			boss.signForm(*rrf);
			boss.executeForm(*rrf);
